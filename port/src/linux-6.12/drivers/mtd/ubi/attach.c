			...
			int pnum, const struct ubi_vid_hdr *vid_hdr)
{
	...
	if (sqnum2 == aeb->sqnum) {
		struct ubi_vid_io_buf *old_vidb;
		struct ubi_vid_hdr *old_vid;
		__be32 raw_v1, raw_v2;
		long long delta;
		uint32_t v1, v2;

		/*
		 * The DG2200TN vendor image uses the pre-sqnum UBI format from
		 * before UBI entered mainline. Its 32-bit LEB version occupies
		 * what is padding1[4] in the current on-flash structure.
		 * Restore the comparison used by Linux 2.6.22 so an unclean
		 * vendor shutdown with duplicate LEBs can still be attached.
		 */
		old_vidb = ubi_alloc_vid_buf(ubi, GFP_KERNEL);
		if (!old_vidb)
			return -ENOMEM;
		old_vid = ubi_get_vid_hdr(old_vidb);

		err = ubi_io_read_vid_hdr(ubi, aeb->pnum, old_vidb, 0);
		if (err && err != UBI_IO_BITFLIPS) {
			ubi_free_vid_buf(old_vidb);
			return err > 0 ? -EIO : err;
		}

		memcpy(&raw_v1, old_vid->padding1, sizeof(raw_v1));
		memcpy(&raw_v2, vid_hdr->padding1, sizeof(raw_v2));
		v1 = be32_to_cpu(raw_v1);
		v2 = be32_to_cpu(raw_v2);
		ubi_free_vid_buf(old_vidb);

		delta = (long long)v1 - v2;
		if (delta < 0)
			delta = -delta;
		  ...
		  int ec, const struct ubi_vid_hdr *vid_hdr, int bitflips)
{
	...
	while (*p) {
		...
		if (aeb->sqnum == sqnum && sqnum != 0) {
			/*
			 * Broadcom's vendor UBI writes redundant layout-volume
			 * copies with the same sequence number. Select the copy
			 * indicated by the legacy LEB version, but preserve the
			 * other PEB as alien so writable recovery never erases
			 * vendor fallback data.
			 */
			preserve_duplicate = true;
			ubi_warn(ubi,
				 "preserve duplicate sequence number %llu in compatibility mode",
				 sqnum);
		}
