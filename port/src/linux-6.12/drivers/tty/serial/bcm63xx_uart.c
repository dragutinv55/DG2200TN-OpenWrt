...
static int bcm_uart_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "DG2200TN bring-up: probing BCM63xx UART\n");
	...
	if (pdev->id < 0 || pdev->id >= BCM63XX_NR_UARTS) {
		dev_err(&pdev->dev, "invalid UART alias id %d\n", pdev->id);
		return -EINVAL;
	}
	...
	if (IS_ERR(clk)) {
		dev_err(&pdev->dev, "failed to get UART clock: %ld\n",
			PTR_ERR(clk));
		return -ENODEV;
	}
	...
	if (ret) {
		dev_err(&pdev->dev, "failed to register ttyS%d: %d\n",
			pdev->id, ret);
		ports[pdev->id].membase = NULL;
		return ret;
	}
	dev_info(&pdev->dev, "registered ttyS%d at %pa, irq %d, clock %u\n",
		 pdev->id, &port->mapbase, port->irq, port->uartclk);
