...
#define PCIE_CPU_INTR1_MASK_CLEAR_63138	0x940c
#define PCIE_CPU_INTR1_INTX_63138	GENMASK(4, 1)
...
static int brcm_pcie_bridge_sw_init_set_63138(struct brcm_pcie *pcie, u32 val)
{
	return 0;
}
				      ...
				      struct inbound_win inbound_wins[])
{
	...
	if (pcie->soc_base == BCM63138) {
		/*
		 * BCM63138 routes endpoint DMA through RC BAR1 and requires
		 * the matching UBUS remap window to be enabled.
		 */
				      ...
				      u8 num_inbound_wins)
{
	...
	for (i = 1; i <= num_inbound_wins; i++) {
		 ...
		 */
		if (pcie->soc_base == BCM7712 || pcie->soc_base == BCM63138) {
...
static int brcm_pcie_setup(struct brcm_pcie *pcie)
{
	...
	if (pcie->soc_base == BCM63138) {
		writel(PCIE_CPU_INTR1_INTX_63138,
		       base + PCIE_CPU_INTR1_MASK_CLEAR_63138);
	}
...
static void brcm_extend_rbus_timeout(struct brcm_pcie *pcie)
{
	...
	/* BCM63138 and BCM7712 do not have this (RGR1) timer. */
	if (pcie->soc_base == BCM63138 || pcie->soc_base == BCM7712)
		return;
...
static const int pcie_offsets_bcm63138[] = {
	[EXT_CFG_INDEX]		= 0x9000,
	[EXT_CFG_DATA]		= 0x8000,
	[PCIE_HARD_DEBUG]	= 0x4204,
	[PCIE_INTR2_CPU_BASE]	= 0x4300,
...
static const struct pcie_cfg_data bcm63138_cfg = {
	.offsets	= pcie_offsets_bcm63138,
	.soc_base	= BCM63138,
	.perst_set	= brcm_pcie_perst_set_generic,
	.bridge_sw_init_set = brcm_pcie_bridge_sw_init_set_63138,
	.num_inbound_wins = 3,
};
...
static const struct of_device_id brcm_pcie_match[] = {
	{ .compatible = "brcm,bcm63138-pcie", .data = &bcm63138_cfg },
...
static int brcm_pcie_probe(struct platform_device *pdev)
{
	...
	if (pcie->soc_base == BCM63138) {
		...
		misc = syscon_regmap_lookup_by_phandle(np, "brcm,misc");
		if (IS_ERR(misc)) {
			ret = dev_err_probe(&pdev->dev, PTR_ERR(misc),
					    "failed to get misc reset controller\n");
			goto fail_clk;
		}

		ret = of_property_read_u32(np, "brcm,pcie-index", &index);
		if (ret) {
			dev_err(&pdev->dev, "missing brcm,pcie-index\n");
			goto fail_clk;
		}

		/* Match the vendor reset sequence in miscPCIECtrl. */
		regmap_update_bits(misc, 0, BIT(index), 0);
		msleep(20);
		regmap_update_bits(misc, 0, BIT(index), BIT(index));
		msleep(500);
	}
	...
	ret = reset_control_reset(pcie->rescal);
	if (ret) {
		ret = dev_err_probe(&pdev->dev, ret, "failed to deassert 'rescal'\n");
		goto fail_clk;
	}
