...
#define   USB_CTRL_EBRIDGE_EHCI_ENDIAN_SWAP_MASK	BIT(3)
#define   USB_CTRL_EBRIDGE_OHCI_ENDIAN_SWAP_MASK	BIT(1)
...
#define USB_CTRL_PLL_LDO_CTL		0x40
#define   USB_CTRL_PLL_LDO_CTL_AFE_BG_PWR_MASK		BIT(0)
#define   USB_CTRL_PLL_LDO_CTL_LDO_PWR_MASK		BIT(1)
...
static void usb63138_manual_ldo_start(void __iomem *ctrl)
{
	u32 reg;

	USB_CTRL_UNSET(ctrl, PLL_CTL, PLL_RESETB);
	brcm_usb_writel(0, USB_CTRL_REG(ctrl, UTMI_CTL_1));
	brcm_usb_writel(4, ctrl + USB_CTRL_PLL_LDO_CTL);

	reg = brcm_usb_readl(USB_CTRL_REG(ctrl, PLL_CTL));
	brcm_usb_writel(reg | BIT(31), USB_CTRL_REG(ctrl, PLL_CTL));
	msleep(10);
	brcm_usb_writel(reg & ~BIT(31), USB_CTRL_REG(ctrl, PLL_CTL));

	reg = brcm_usb_readl(ctrl + USB_CTRL_PLL_LDO_CTL);
	reg |= USB_CTRL_PLL_LDO_CTL_AFE_BG_PWR_MASK |
	       USB_CTRL_PLL_LDO_CTL_LDO_PWR_MASK;
	brcm_usb_writel(reg, ctrl + USB_CTRL_PLL_LDO_CTL);
	msleep(1);

	brcm_usb_writel(0x00020002, USB_CTRL_REG(ctrl, UTMI_CTL_1));
	USB_CTRL_SET(ctrl, PLL_CTL, PLL_RESETB);
	msleep(10);
}

static void usb_init_common_63138(struct brcm_usb_init_params *params)
{
	void __iomem *ctrl = params->regs[BRCM_REGS_CTRL];

	USB_CTRL_SET(ctrl, SETUP, OC_DISABLE_PORT0);
	USB_CTRL_SET_FAMILY(params, SETUP, OC3_DISABLE_PORT0);
	brcmusb_usb3_ssc_enable(ctrl);
	msleep(300);
	usb63138_manual_ldo_start(ctrl);
	brcmusb_usb2_eye_fix(ctrl);
}

static void usb_init_eohci_63138(struct brcm_usb_init_params *params)
{
	void __iomem *ctrl = params->regs[BRCM_REGS_CTRL];
	u32 reg;

	reg = brcm_usb_readl(USB_CTRL_REG(ctrl, EBRIDGE));
	reg &= ~(USB_CTRL_EBRIDGE_EHCI_ENDIAN_SWAP_MASK |
		 USB_CTRL_EBRIDGE_OHCI_ENDIAN_SWAP_MASK);
	brcm_usb_writel(reg, USB_CTRL_REG(ctrl, EBRIDGE));
}

static void usb_init_xhci_63138(struct brcm_usb_init_params *params)
{
	void __iomem *ctrl = params->regs[BRCM_REGS_CTRL];
...
static void usb_uninit_xhci_63138(struct brcm_usb_init_params *params)
{
	USB_CTRL_UNSET_FAMILY(params, USB_PM, XHC_SOFT_RESETB);
}
...
static const struct brcm_usb_init_ops bcm63138_ops = {
	.init_ipp = usb_init_ipp,
	.init_common = usb_init_common_63138,
	.init_eohci = usb_init_eohci_63138,
	.init_xhci = usb_init_xhci_63138,
	.uninit_common = usb_uninit_common,
	.uninit_eohci = usb_uninit_eohci,
	.uninit_xhci = usb_uninit_xhci_63138,
	.get_dual_select = usb_get_dual_select,
	.set_dual_select = usb_set_dual_select,
};

void brcm_usb_dvr_init_63138(struct brcm_usb_init_params *params)
{
	params->selected_family = BRCM_FAMILY_63138;
	params->usb_reg_bits_map =
		&usb_reg_bits_map_table[BRCM_FAMILY_63138][0];
	params->family_name = family_names[BRCM_FAMILY_63138];
	params->ops = &bcm63138_ops;
}
