...
#ifdef RDD_BASIC
    /*
     * RDD_BASIC keeps EMAC1 in normal per-MAC scheduling mode, so BBH1
     * completions must wake the same dedicated task stored in its MAC
     * descriptor.  Waking the inter-LAN task leaves one packet queued until
     * the next CPU-TX enqueue generates another wakeup.
     */
...
static uint8_t f_bbh_rx_runner_task_normal_queue(DRV_IH_RUNNER_CLUSTER runner_cluster, DRV_BBH_PORT_INDEX port_index)
{
    ...
    else
    {
        ...
        else
        {
            ...
            case DRV_BBH_EMAC_1:
#if defined(_CFE_) || defined(RDD_BASIC)
...
static void f_configure_runner(S_DPI_CFG  *pCfg, rdp_bpm_cfg_params *p_bpm_cfg_params)
{
    ...
    /* Stock Linux uses the SF2 through Runner's Broadcom switch mode. */
    rdd_init_params.broadcom_switch_mode = 1;
