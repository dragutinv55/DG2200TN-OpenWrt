...
#define CACHED_MALLOC_ATOMIC(_size) 			uncache_to_cache((uint32_t)KMALLOC((_size),0))
#define CACHED_MALLOC(_size) 				uncache_to_cache((uint32_t)KMALLOC((_size),0))
#define NONCACHED_MALLOC_ATOMIC(_size) 			cache_to_uncache((uint32_t)KMALLOC((_size),0))
...
#define NONCACHED_MALLOC_ATOMIC(_size, _phys_addr_ptr)	({void *_acp_addr_ptr = NULL;			\
		void *_org_addr_ptr = dma_alloc_coherent(NULL, _size + 4, _phys_addr_ptr, GFP_ATOMIC|GFP_ACP);	\
		if (_org_addr_ptr != NULL) {								\
			_acp_addr_ptr = (void *)ACP_ADDRESS(*_phys_addr_ptr);				\
			*(uint32_t *)((uint32_t)_acp_addr_ptr + _size) = (uint32_t)_org_addr_ptr;	\
		}											\
...
#define NONCACHED_MALLOC_ATOMIC(_size, _phys_addr_ptr)	\
	bcm63138_rdp_dma_alloc(_size, _phys_addr_ptr, GFP_KERNEL)
#define NONCACHED_MALLOC(_size, _phys_addr_ptr)		\
	bcm63138_rdp_dma_alloc(_size, _phys_addr_ptr, GFP_KERNEL)
#define NONCACHED_FREE(_size, _ptr, _phys_addr)		\
	bcm63138_rdp_dma_free(_size, _ptr, _phys_addr)
#else
