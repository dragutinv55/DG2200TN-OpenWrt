...
void rdd_exit ( void )
{
    ...
    g_cpu_tx_data_pointers_reference_array = NULL;
    if ( g_free_skb_indexes_fifo_table )
        alignedFree( ( void * )g_free_skb_indexes_fifo_table, sizeof( uint16_t ) * g_cpu_tx_abs_packet_limit );
    g_free_skb_indexes_fifo_table = NULL;
    g_free_skb_indexes_fifo_table_physical_address = NULL;
    g_free_skb_indexes_fifo_table_physical_address_last_idx = NULL;
    g_cpu_tx_abs_packet_limit = 0;
#endif
