...
BL_LILAC_RDD_ERROR_DTE rdd_cpu_tx_initialize ( void )
{
    ...
    for (i = 0; i < 4; i++)
        g_cpu_tx_queue_free_counter[i] = 0;
...
#if !defined(FIRMWARE_INIT) && (!defined(RDD_BASIC) || defined(BCM63138_RDP_MAINLINE))
...
#if !defined(RDD_BASIC) || defined(BCM63138_RDP_MAINLINE)
                                                              ...
                                                              uint8_t                       xi_exclusive_packet )
{
    ...
    {
        bpm_src_port = rdd_bridge_port_to_bpm_src_port ( g_broadcom_switch_physical_port );
    }
    else
    {
        bpm_src_port = rdd_bridge_port_to_bpm_src_port ( xi_src_bridge_port );
    }
                                                              ...
                                                              uint32_t  xi_downstream_gem_flow )
{
    ...
    {
        bpm_src_port = rdd_bridge_port_to_bpm_src_port ( BL_LILAC_RDD_WAN0_BRIDGE_PORT ); // DSL
    }
    else if ( g_wan_physical_port == BL_LILAC_RDD_WAN_PHYSICAL_PORT_ETH4 )
    {
        bpm_src_port = rdd_bridge_port_to_bpm_src_port ( BL_LILAC_RDD_LAN4_BRIDGE_PORT );
    }
    else if ( g_wan_physical_port == BL_LILAC_RDD_WAN_PHYSICAL_PORT_ETH0 )
    {
        bpm_src_port = rdd_bridge_port_to_bpm_src_port ( BL_LILAC_RDD_LAN0_BRIDGE_PORT );
    }
    else
    {
        f_rdd_unlock_irq ( &int_lock_irq, flags );
